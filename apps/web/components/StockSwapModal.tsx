...
import { STOCK_ISSUER_LABEL, type StockToken } from '@/lib/lifi/stocks';
...
export function StockSwapModal({
  token,
  initialSide,
  onClose,
}: {
  ...
  const stockBal = useReadContract({
  ...
  const maxAmount = side === 'buy'
    ...
    : ((stockBal.data as bigint | undefined) ?? 0n);
    ...
    <div className="fixed inset-0 z-[200] flex items-end sm:items-center justify-center bg-black/60 sm:px-4">
      <div className="w-full max-w-md bg-paper border border-border border-b-0 sm:border-b rounded-t-2xl sm:rounded-lg p-6 max-h-[min(92dvh,100%)] overflow-y-auto pb-[max(1.5rem,env(safe-area-inset-bottom))]">
        <div className="flex items-start justify-between mb-4">
          <div>
            ...
            <div className="text-sm text-ink/50">
              {token.name} · {STOCK_ISSUER_LABEL[token.issuer]} · {stockChainLabel(token.chainId)}
            </div>
          </div>
        ...
        <p className="text-xs text-ink/50 leading-relaxed mb-4">
          Tokenized representation routed by LI.FI — not the listed share. Not advice. You sign
          the swap. Openhand never holds the tokens. Availability depends on the issuer.
        </p>

        {!mainnet && (
          <div className="text-xs text-warn bg-warn/10 border border-warn/30 rounded px-3 py-2 mb-4">
            Practice mode. Tokenized stock swaps are mainnet-only.
          </div>
        )}

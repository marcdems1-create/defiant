...
export function DepositWithdrawModal({
  ...
  // Morpho vault shares aren't 1:1 with the underlying, so that withdraw tab
  // enters an amount of positionToken. Moonwell's
  ...
  // Sky withdrawals are sUSDS shares, not 1:1 with USDC — the fee must be
  // computed on the previewed USDC output instead, or it comes out wildly
  // wrong (off by orders of magnitude). Aave/Yearn keep using amountBig
  // directly: their share/aToken amounts are a close enough proxy for the
  // underlying that this is fine there.
  ...
  async function handleDeposit() {
    ...
    try {
      ...
      // Aave and the ERC-4626 protocols: ERC-20 approve for the exact net
      // deposit amount, then act. Deliberately not an unbounded approval —
      // scoped to this deposit only, and scoped to the post-fee amount since

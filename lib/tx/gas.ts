...
/** Dust ETH that still covers a couple of Base/Arbitrum ERC-20 txs. Below this we pay gas in USDC. */
...
export function formatTxError(error: unknown): string {
  ...
  if (/cannot pay gas in USDC|missing EIP-7702/i.test(blob)) {
    return tidyMessage(blob).slice(0, 220);
  }
  if (/AA23|paymaster.*revert|paymaster signature|invalid permit/i.test(blob)) {
    return 'USDC gas payment failed. Check the USDC permit in your wallet and try again.';
  }
  if (
    /insufficient funds|exceeds the balance of the account|insufficient .*balance/i.test(blob) &&
    /gas|fee|intrinsic/i.test(blob)
  ) {
    return 'Not enough ETH for gas, and USDC gas could not be used. Leave a little USDC in the wallet or send ETH, then try again.';
  }

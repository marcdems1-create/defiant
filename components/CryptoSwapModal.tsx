...
export function CryptoSwapModal({
  token,
  initialSide,
  onClose,
}: {
  ...
  const sellingNative = side === 'sell' && token.native;
  ...
  const tokenBal = useReadContract({
    ...
    args: address ? [address] : undefined,
    chainId: token.chainId,
    query: { enabled: Boolean(address && mainnet && !token.native) },
  });
  ...
  const { data: allowance } = useReadContract({
    ...
    args: address && quote?.approvalAddress && sellToken && !sellingNative
      ? [address, quote.approvalAddress]
      : undefined,
    chainId: token.chainId,
    query: {
      enabled: Boolean(
        address && sellToken && quote?.approvalAddress && mainnet && !sellingNative,
      ),
    },
  });
  ...
  async function submit() {
    ...
    try {
      const needsApprove =
        !sellingNative && ((allowance as bigint | undefined) ?? 0n) < parsedSell;
      if (needsApprove) {
        setStep('approving');
    ...
    <div className="fixed inset-0 z-[200] flex items-end sm:items-center justify-center bg-black/60 sm:px-4">
      <div className="w-full max-w-md bg-paper border border-border border-b-0 sm:border-b rounded-t-2xl sm:rounded-lg p-6 max-h-[min(92dvh,100%)] overflow-y-auto pb-[max(1.5rem,env(safe-area-inset-bottom))]">
        <div className="flex items-start justify-between mb-4">
          <div>
            ...
            <div className="text-sm text-ink/50">
              {token.name} · {stockChainLabel(token.chainId)}
            </div>
          </div>
        ...
        <p className="text-xs text-ink/50 leading-relaxed mb-4">
          Spot crypto routed by LI.FI. Not advice. You sign the swap. Openhand never holds the
          tokens.
        </p>

        {!mainnet && (
          <div className="text-xs text-warn bg-warn/10 border border-warn/30 rounded px-3 py-2 mb-4">
            Practice mode. Crypto swaps are mainnet-only.
          </div>
        )}
